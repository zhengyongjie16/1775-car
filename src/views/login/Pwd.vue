  ...
  <div class="main">
    ...
    <div class="content">
      ...
      <div class="foot" v-show="isFous==false">
        <div class="btn" @click="accountReg" >重置密码</div>
      </div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    mounted(){
       window.onresize = ()=> { // 定义窗口大小变更通知事件
           this.screenHeight1 = document.body.clientHeight || window.screen.height;
       }
    },
    watch: {
      isFous:(value)=>{
    ...
    methods: {
      ...
      async accountReg(){
        ...
        let res = await this.$api.post('/bus/companyGetPwd',{
          phone:this.phone,
          code:this.code,
          newPwd:this.pwd
        });
        ...
        if(!success){
          ...
          await this.$toast.info(info,1000);
          this.out();
        }
      },
      out(){
