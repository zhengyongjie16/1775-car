  ...
  <div class="MyCar">
    ...
    <div class="MyCarBox">
      ...
      <!--车辆显示列表-->
          ...
          <md-scroll-view
            ref="scrollView"
            :scrolling-x="false"
            @refreshing="onRefresh"  v-if="CarTableData"
            :autoReflow="true"
            class="CarDataLi"
            @endReached="getList"
          >
            ...
            ></md-scroll-view-refresh>
            <ul>
              <li v-for="(item,r) in CarTableDataLi"     :key="item.id">
              <!--车辆信息-->
              <h1 @click="CarDataRou(item)">{{item.title}}</h1>
              <div class="CarinformationTh">
                <div class="ciLeft">
                  <div class="CarDataLe">
                    <div class="CarDataLeTu" @click="CarDataRou(item)">
                      <img :src="item.attachments.length>0? mobilePic(item.attachments[0].path):''" alt="">
                    </div>
                    <!--<div class="CarDataLeFont">视频未上传</div>-->
                  </div>
                  <div class="CarDataRi">
                    <p>上牌时间：{{new Date(item.brandDate)|moment('YYYY-MM-DD')}}</p>
                    <p>里程：{{item.mileage}}万公里</p>
                    <p>网销价格: {{item.onlinePrice}}万</p>
                    <p style="font-weight: 600;color:#ff6600">批发价格: {{item.tradePrice}}万</p>
                    <p>车源ID：{{item.id}}</p>
                  </div>
                </div>

                <div class="np">
                  <div>车源归属: {{item.company.companyName}}</div>
                  <div>联系人: {{item.company.lxName}} {{item.company.lxPhone}}</div>
                </div>
                <a class="tel" :href="`tel:${item.company.lxPhone}`" @click.stop="doNT">
    ...
    <van-popup v-model="sxBox" position="right" :style="{width:'100vw',height:'100%'}" class="mypop">
      ...
      <div class="MyCarSele">
        ...
        <div>
          <span>库龄:</span>
          <el-select class="checkSele" size="mini" v-model="Choage" placeholder="不限" @change="getList">
            <el-option
        ...
        <div>
          <span>里程:</span>
          <el-select class="checkSele" size="mini" v-model="Chomileage" placeholder="不限" @change="getList">
            <el-option
        ...
        <div>
          <span>是否批发车源:</span>
          <el-select class="checkSele" size="mini" v-model="Wholesale" placeholder="全部" @change="getList">
            <el-option
        ...
        <md-button  style="margin-top: 20px;"  @click="doSearch">查找</md-button>
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      // 快速查找
      if (this.fastSearch) _postData.title = this.fastSearch;
      // 是否是批发车辆
      ...
      if (success) {
        data.list.map((r) => {
          if (!this.CarTableDataLi.find(item => item.id == r.id)) this.CarTableDataLi.push(r);
        });
        if (data.list.length == 0) {
          this.isFinished = true; // 没有更多的内容可以加载了
        }
